...
export default async function Products({ searchParams }: { searchParams: Promise<ISearchParams> }) {
        ...
        <div className="max-w-7xl mx-auto p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mt-8">
                {products.map((product) => (
                    <div key={product.id} className="flex flex-col gap-3 shadow-md rounded-xl overflow-hidden bg-white hover:shadow-xl transition-shadow duration-300">
                        <a href={`/products/${product.id}`} className="bg-gray-100 h-40">
                            <img src={product.image} alt="image" />
                        </a>
                        <div className="p-4 flex flex-col">
                            <a href={`products/${product.id}`} className="font-bold hover:text-green-600 transition-colors duration-300 cursor-pointer">{product.name}</a>
